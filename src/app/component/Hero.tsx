"use client";
import { motion } from "framer-motion";
import Link from "next/link";
import type { FC } from "react";

const Hero: FC = () => {
  return (
    <section
      id="hero"
      className="relative h-[80vh] flex items-center justify-center text-center text-white overflow-hidden"
    >
      <div className="absolute inset-0 z-0">
        <video
          autoPlay
          muted
          loop
          playsInline
          className="w-full h-full object-cover"
        >
          <source src="/media/WhatsApp Video 2025-08-22 at 5.32.44 AM (1).mp4" type="video/mp4" />
        </video>
        <div className="absolute inset-0 bg-black/50" />
      </div>

      <div className="relative z-10 px-4">
        <h1 className="text-4xl text-blue-200 md:text-6xl font-bold mb-4">
          A Family in Christ on Campus
        </h1>
        <p className="text-lg md:text-xl mb-8 text-blue-100">
          Join us for worship, prayer, and impact
        </p>
        <Link href="#about">
          <motion.span
            className="inline-block px-8 py-3 text-lg font-bold text-black bg-gradient-to-tr from-blue-300 to-teal-50 rounded-full shadow-lg hover:from-blue-900 hover:to-white hover:-translate-y-1 transition cursor-pointer"
            animate={{ scale: [1, 1.1, 1] }}
            transition={{ duration: 1, repeat: Infinity, repeatType: "loop", ease: "easeInOut" }}
          >
            Join Us
          </motion.span>
        </Link>
      </div>
    </section>
  );
};

export default Hero;
